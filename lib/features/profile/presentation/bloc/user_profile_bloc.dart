...
class UserProfileBloc extends Cubit<UserInfoState>{
  ...
  UserProfileBloc(this.getUserUseCase,this.updateUserUseCase):super(GetUserStateIdle());
     getUserData()async{
      ...
      updateUserData(UpdateUserRequest model)async{
        emit(UpdateUserInfoStateLoading());
        var either = await updateUserUseCase.updateUser(model);
        either.fold(
                (l) {

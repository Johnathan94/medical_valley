import 'package:flutter/material.dart';

EdgeInsetsDirectional smallPaddingAll =  const EdgeInsetsDirectional.all(8);
EdgeInsetsDirectional mediumPaddingAll =  const EdgeInsetsDirectional.all(16);
EdgeInsetsDirectional bigPaddingAll =  const EdgeInsetsDirectional.all(32);
EdgeInsets smallPaddingH =  const EdgeInsets.symmetric(horizontal: 8 );
EdgeInsets mediumPaddingH =  const EdgeInsets.symmetric(horizontal: 16 );
EdgeInsets bigPaddingH =  const EdgeInsets.symmetric(horizontal: 24 );
EdgeInsets largePaddingH =  const EdgeInsets.symmetric(horizontal: 32 );

EdgeInsets smallPaddingHV =  const EdgeInsets.symmetric(horizontal: 8 , vertical: 8);
EdgeInsets mediumPaddingHV =  const EdgeInsets.symmetric(horizontal: 16 , vertical: 16);
EdgeInsets bigPaddingHV =  const EdgeInsets.symmetric(horizontal: 24 , vertical: 24);
EdgeInsets largePaddingHV =  const EdgeInsets.symmetric(horizontal: 32 , vertical: 32);

EdgeInsets smallPaddingV =  const EdgeInsets.symmetric(horizontal: 8 );
EdgeInsets mediumPaddingV =  const EdgeInsets.symmetric(horizontal: 16 );
EdgeInsets bigPaddingV =  const EdgeInsets.symmetric(horizontal: 24 );
EdgeInsets largePaddingV =  const EdgeInsets.symmetric(horizontal: 32 );

